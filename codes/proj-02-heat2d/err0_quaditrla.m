...
function [errrr0, errrr1, hx]=error0(nn)
...
for ee = 1:n_el
    ...
    for ll = 1 : npq*npq % totsl number of quadrature points per element
        ...
        for aa = 1:n_en
            xx_l = xx_l + xx_ele(aa)*Quad(aa,xii(ll),etaa(ll));
            yy_l = yy_l + yy_ele(aa)*Quad(aa,xii(ll),etaa(ll));
            uh = uh + u_ele(aa)*Quad(aa,xii(ll),etaa(ll));
            [Na_xii, Na_etaa] = Quad_grad(aa, xii(ll), etaa(ll));
