...
function [el2,hh]=error1(ele_num)
...
% Setup the mesh
pp   = 2;        % polynomial degree while the number of element is changing
n_en = pp + 1;         % number of local nodes in one element
n_el = ele_num;              % number of elements
...
for ee = 1 : n_el
  ...
  for ll = 1 : n_sam_end
    ...
    for aa = 1 : n_en
      ...
      u_l = u_l + u_ele(aa) * PolyShape(pp, aa, xi_sam(ll), 1)*2*n_el;
    end

    x_sam( (ee-1)*n_sam + ll ) = x_l;
    u_sam( (ee-1)*n_sam + ll ) = u_l;
    y_sam( (ee-1)*n_sam + ll ) = 5*x_l^4;
  end
end
...
n_inte=length(x_sam);
[x_i,w]=Gauss(n_inte,0,1);
int1=0;
int2=0;

for i=1:n_inte
    int1=int1+w(i)*((u_sam(i)-y_sam(i))^2);
    int2=int2+w(i)*(y_sam(i)^2);
end
el2=int1^(0.5)/int2^(0.5);
end